...
用户已授权先实现已知 UI 目标，再把未知的最终美术、字体、时序与配置问题留到末尾确认；实现过程仍不等于最终视觉批准。

1. 产品实现与 docs PR #35 分离；#35 只归档文档，不包含产品实现，也不代表最终视觉批准。本 docs 分支保留来源追溯，不把产品分支、PR #37 或历史 PR 的状态写成本文档分支的完成证据。
2. 当前实现目标是恢复**截图级的 TH 页面关系**：标题、人物／场景、背景、装饰与文字焦点共同成景，且跨主菜单、难度、自机选择等页面家族保持同一舞台语法；旧浅纸／米纸网站化基线仅作历史／局部材质 provenance，不再作为全局默认。
3. **第一切片仅含主菜单、难度选择、自机选择**。主菜单使用非对称完整标题场景与无框文字导航；难度页使用错位的大号英文等级／名称和可读中文说明／元数据；自机页以当前 `.selected` 立绘为唯一视觉焦点，另一位自机仍是真实可点击／可键盘操作的无框名称与说明。战斗、暂停、结果、加载和面间／stage 转换不在本切片内，不改 overlay selector。
4. 桌面优先对齐截图关系；窄屏必须重排并在 390px、320px 宽度保持可读、焦点可达和返回路径可用。完整页面时间线可由 Z／确认键中断到完成态；`prefers-reduced-motion` 直接呈现完成态。逻辑、真实数据、输入与录像／录制契约保持不变。
5. 运行时抽象 AVIF 资产如后续进入正式候选，单文件不超过 4MB、总量不超过 32MB，桌面源图至少 1920×1080，并记录 prompt、来源、日期、工具、许可证和 SHA-256。Dream Han 仅是候选字体，须先完成官方上游与许可证核验；在此之前不写成已采用字体。
6. 当前第一切片交付门槛是三页各自的桌面、390px、320px 截图，真实键盘／鼠标确认与返回路径，以及 `npm` 逻辑检查和相关 E2E 证据。门槛是待交付证据，不是视觉通过或实现完成声明。
7. 保留 U1 自机配置、最终人物／背景／字体、精确时序、资产授权与录像兼容等未决边界；现有素材与字体可作阶段性实现材料，不得据此宣称最终美术、视觉批准或完整前端完成。
