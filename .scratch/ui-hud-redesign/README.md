...
3. 主菜单和选择页本轮只更新规划目标，不扩大代码范围；暂停／结果层才是本轮代码调整边界。新目标是后续实现依据，不代表已经实现或获得最终视觉批准。
4. 保留 02/06 已交付的功能与结构，先按上述目标调整已实现 UI 的风格并单独记录用户视觉结论。20 是战斗 UI 的现有调整入口；主菜单的调整范围需沿用 02 记录，不能擅自认为 20 覆盖全部菜单。
5. 风格结论确认后，按真实依赖继续剩余 UI 票。08 还依赖未登记的 07；11 原本无功能前置，现在也等待上述风格调整。功能测试成功不自动解除视觉阻塞。
本轮新目标是规划层依据，不是已实现或视觉已批准结论；截图、历史原型和既有 CI 结果仍仅作 provenance，不能替代本轮暂停／战败状态层的真实渲染审阅。人物资产、自机配置、最终字体等 U1/U2/P1 边界继续保留。
