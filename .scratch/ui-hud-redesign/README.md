...
2. 旧“浅色米纸网站化”基线降级为历史／局部材质参考，不再作为全局构图或后续实现默认。当前后续实现的第一优先级改为 TH 原作式页面级构图、无框文字菜单、场景内状态叠层。暂停／战败保留左侧战斗区、右侧 HUD 与作品标题，仅局部压暗／柔化左侧战斗区；状态标题位于选项组上方，操作选项锚定左下并纵向排列。
3. 主菜单、选择页、战斗 HUD、暂停／战败／结果状态等已知风格目标均可按上述方向进入后续实现；这些目标是实现依据，不代表任何页面已实现或获得视觉批准。
4. 保留 02/06 已交付的功能与结构，先按上述目标调整已实现 UI 的风格并单独记录用户视觉结论。20 是战斗 UI 的现有调整入口；主菜单的调整范围需沿用 02 记录，不能擅自认为 20 覆盖全部菜单。
5. 按真实依赖继续剩余 UI 票；视觉通过仍待用户确认，功能测试成功不自动解除视觉阻塞。08 还依赖未登记的 07；11 原本无功能前置，现在也等待上述风格调整。
本轮新目标是规划层依据，不是已实现或视觉已批准结论；截图、历史原型和既有 CI 结果仍仅作 provenance，不能替代后续目标页面与状态层的真实渲染审阅。人物资产、自机配置、最终字体等 U1/U2/P1 边界继续保留。
