...
本规格原始来源状态为 `ready-for-agent`；归档后保留为规划与来源记录。当前用户已授权实现已知 UI 目标；未知的美术、字体、时序、人物资产与配置机制留待末尾确认，任何实现都不等于最终视觉批准。
...
- 本文是规划归档，不是 UI 已实现或已验收的证明。已知目标可进入本地实现，但最终美术与视觉审阅仍未收口。
- 历史／当前偏差必须保留在后续实现判断中：现有 Nomiss 玩家页入口与未来目标的“故事难度页独立规则区”不同；未来配置页、难度页和完整 UI 流程不能倒写成现状。
- 08、11 继续记录部分实现与规划目标；本轮仅解除已知视觉目标的实现阻塞，不解锁未指定机制。详见对应票面的 `Status`、`Blocked by` 和 `Implementation` 字段。
- U1 自机配置机制、U2 全套立绘／资产、P1 精确视觉与时序值，以及 N→兼容验证仍是未决项；下文目标、验收矩阵与完成门槛完整保留。
...
暂停／战败保留原战斗版面：标题、正文与动作在版面内形成左下聚类，局部压暗并柔化左侧战斗区，右侧 HUD 与作品标题保留；主菜单使用 TH 式非对称页面构图，选择页使用页题、当前项、说明与角色／场景关系。上述已知目标可进入本地实现，但不等于最终视觉批准。

战斗始终优先中央 `450×600` 逻辑游戏区、真实数据与可玩性；桌面／手机排版、输入、模式例外、录像、真实存档和状态契约保持不变。前端结构与交互可以先使用明确占位推进；当前选择与当前立绘按已有真实状态与资源呈现，缺失资产、全套立绘与自机配置机制仍受 U2／U1 边界约束，不能由视觉目标虚构补齐。
...
- **主菜单与选择页目标（N）**：主菜单使用 TH 式非对称页面构图；选择页使用页题、当前项、说明与角色／场景关系。两者使用无框文字、明暗、描边、位置或短刷痕表达焦点，不使用现代按钮框或左侧竖线选中态。可按当前方向进入后续实现，但不把目标写成已实现或视觉已批准；未定人物与自机配置仍受 U2／U1 约束。
...
### ID-02A · 当前第一切片（N，截图级关系）

- **范围**：当前实现目标只包含主菜单、难度选择、自机选择三页。战斗、暂停、结果、加载和面间／stage 转换不属于本切片；不得通过本切片改动游戏 overlay selector 或其他页面的状态契约。
- **页面关系**：三页共同恢复截图级 TH 页面关系，保留作品身份与真实数据，让标题、人物／场景、背景、装饰和文字焦点共同成景。主菜单是完整标题场景、非对称构图、无框文字导航与选中焦点；难度页使用错位的大号英文等级／名称，并同时呈现可读中文描述与元数据，不使用卡片、行面板或左侧竖栏；自机页以当前 `.selected` 立绘为唯一视觉焦点，另一位自机仍是实际可点击／可键盘操作的无框名称与说明，ArrowRight 必须交换选中立绘，Nomiss／返回保持原行为。
- **响应式**：桌面是主要保真目标；窄屏重排而非机械缩放，至少覆盖 390px 与 320px 宽度，文字、焦点、确认、返回和角色说明保持可读可达。
- **时间线与输入**：每页完整演出时间线均可由 Z／确认键中断到完成态；`prefers-reduced-motion` 直接得到同一完成态而不丢信息。确认、鼠标点击、方向键和返回路径必须沿用真实输入消费，不改变逻辑尺寸、数据、模式、录制／录像或其他业务行为。
- **运行时抽象资产边界**：候选 AVIF 单文件不超过 4MB、总量不超过 32MB；桌面源图至少 1920×1080。每项候选必须记录 prompt、来源、日期、工具、许可证和 SHA-256；缺少这些 provenance 时只能留在 staging，不得作为正式运行时资产。Dream Han 只是候选字体，须先核验官方上游与许可证，核验完成前不得写成采用字体。
- **当前交付门槛**：三页各有桌面、390px、320px 截图证据；有真实键盘／鼠标确认与返回路径证据；有 `npm` 逻辑检查和相关 E2E 证据。门槛描述待交付证据，不构成视觉批准或实现完成声明。
...
- **当前推断／目标（N）**：暂停和战败都保留原战斗版面；标题、正文与动作在版面内形成左下聚类，局部压暗并柔化左侧战斗区，右侧 HUD 与作品标题继续可见。该布局目标来自最新用户结论，可进入本地实现，但不是最终视觉批准；Dream 只辅助状态层与局部纸墨材质，不扩展为全局规则或 TH 共性。
