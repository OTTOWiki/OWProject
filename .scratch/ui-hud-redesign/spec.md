...
以一套完整的游戏画面语言重做全部 UI / HUD：浅色米纸、水墨远近和笔刷材质为基线，th14 提供非对称标题／人物／菜单构图与纹样拼贴，th18 提供更强的色彩碰撞、装饰叠层和展示字形，《东方梦无垠》提供空间层次、资源分组、暂停与章节刷痕提示。允许高保真借用局部设计机制，不将参考截图整图或参考人物直接放入游戏。

桌面首先形成主视觉；手机竖屏使用同一美术语言独立排版。菜单和选择页可以富于演出，战斗始终优先可玩性、资源与机制状态可读性。版面保持 450×600 逻辑坐标；外围美术、菜单和 HUD 不受这块区域的尺寸限制。

将既定流程接入真实游戏：统一自机选择与自机配置页面、难度页的故事专属 Nomiss 规则区、同页练习选择、暂停设置返回暂停、双侧人物对话、分步结算以及按内容分别设计的记录页。自机配置页面是确定的目标，但其玩法尚未定义，不能靠虚构配置完成上线。

前端结构与交互先使用明确占位推进；全套立绘制作后置，先编制角色／状态／用途资产清单，再统一制作和验收。占位可以证明结构可用，不能证明最终人物、美术或上线交付完成。
...
- 艺术主标题固定为 **棍維何意味**，小型英文副标题为 **OTTOWiki Project**。标题采用日式汉字字形；缺少对应字形时才用台湾繁体字形补缺。该规则仅限标题，不把全站改成繁体或日文。
- 2026-09-10 追加确认：标题五字大小、上下位置错落，「棍維」与「何意味」明确异色；界面更多借鉴原作 UI。可先生成无人物、无文字的暂用魔幻背景，是否最终采用另定；人物仍后置。
- 以浅纸绘卷水墨为基线，th14 负责非对称构图、纹样拼贴和艺术标题，th18 加强多色对比、装饰叠层、展示文字与迷幻感，《东方梦无垠》负责纸墨远近、资源分组及场景内笔刷提示。
- “第三版还行”是草稿认可，不是最终视觉批准。最终不能只交一套更规整的米色网站；视觉强化必须作为实际工作与验收项。
- 优先在主菜单、难度／选择页、标题牌与转场增加演出和装饰；战斗保持关键读数、玩家、敌人和弹幕清楚。
- 允许高保真借用局部构图及组件机制；不把整张参考截图作为运行时背景，不把参考角色、原作标题或字体文件未经核对直接放入本作。
- 英文主、中文辅仅覆盖导航短文本、选择页题与选项，以及 HUD 通用短词；剧情、说明书、设置解释、错误信息、历史详情中文主。特有机制使用已确认领域术语，不创造看似官方的新机制名。
- 字体层级按断点使用明确字号，不随视口宽度连续缩放；中文无负字距和伪斜体。选中态可用字形、明暗、菱形标记或短刷痕，不能用左侧竖线当强调。
- 显示数值采用半角等宽或等宽数字，不通过装饰字体牺牲长分数与倒计时识别。最后字形和材质取舍以实际渲染审阅为准。
...
**暂停**：仅覆盖/压暗版面，桌面外围HUD继续可见。复用同一设置页，记录来源；从暂停关闭设置回到暂停而非恢复战斗。普通模式保留继续、保存录像、设置、返回等有效操作；Nomiss使用继续、暂停结算、设置、重试等其实际动作集；录像回放采用既有回放动作，不能显示无效录制/续关按钮。
