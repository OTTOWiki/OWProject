...
`nomiss` 无伤模式的核心机制仍保留（被弹自动重开当前章、保留资源、禁录像、进度持久化、仅暂停结算/结局结算，不入榜），但当前入口暂时移除，待迁移到难度选择之前后再恢复；不要在当前 UI 或测试中假设存在 Nomiss 复选框入口。）
...
- 自机选择暂用 `assets/ui/player-placeholder.svg`，饮泉淡青、沙玛淡紫；旧立绘保留但不用于此页。当前人物左侧放大、说明右侧，另一人物缩小模糊并在左缘露半身。角色切换450ms。难度确认先360ms内闪三次，再760ms将难度项与黑带向左移出并向屏幕高度中线收拢；旧标题淡出左移，新标题560ms淡入左移，人物容器450ms从右入场，统一使用 `cubic-bezier(.22,1,.36,1)`。共享彩窗与遮罩固定；转场期间锁定确认，Esc/X取消回难度；减少动态效果时立即切换。仅两名角色参与导航，姓名颜色稳定；点击预览先切换，确认当前角色开局。返回无按钮，使用Esc/X。Nomiss入口暂移除，后续前置计划见队列H02。
- 自机确认当前角色后360ms内闪烁三次再开局；预览点击只切换。闪烁期间锁定重复确认，Esc/X或离开页面取消待启动动作；减少动态效果时立即开局。