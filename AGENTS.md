...
- 自机选择使用两色空白人形SVG，角色切换450ms。模式↔难度↔自机转场760ms，缓动 `cubic-bezier(.22,1,.36,1)`，前进先360ms闪三次。进入或返回途中Esc/X均采样当前computed位置、透明度、宽高和角度，取消旧动画并向另一端点重新以同一先快后慢曲线播放；按住不重复改变方向，异步完成以generation防止旧动画提前收尾。自机动画作用于实际人物和可见文字，屏外人物不可见。三页真实黑带 `.selection-focus-band` 在转场时隐藏，由唯一 `.selection-transition-band` 插值中心、宽高和独立rotate，前进+360度、返回−360度再补目标角差。黑带始终在两页内容下方，自机带宽与说明区一致。完成后清除过渡元素恢复真实黑带。减少动态效果时立即切换。