...
- 选择页转场采用独立三层：`#app` 下固定背景z0、唯一过渡黑带z1、两页透明内容z2；模式列表也明确为内容层，避免返回时被旧页背景遮挡。黑带外框只插值中心与宽高，内层独立rotate，真实目标角度在初始化时关闭CSS transition后测量，结束无位置跳变。进入／返回途中Esc/X都采样当前computed几何，重新以760ms `cubic-bezier(.22,1,.36,1)` 移向另一端点；generation防止旧动画取消时提前收尾。前进先360ms闪三次，闪烁期间可取消，减少动态效果时直接切换。自机使用两色SVG，说明区黑带桌面46vw／手机50vw。模式支持上下／左右切换，Nomiss位于难度前。