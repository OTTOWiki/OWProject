...
- 当前产品 PR 分支干净地直接基于 `origin/main`；`f723d09` 仅作为推导已知目标内容的旧首轮 UI／内容历史来源，不是当前基线。它与开放的 #34 及另行记录的 #35 区分，不代表任一 PR 已合并。
- 暂停与结果状态保留战斗场景：标题、正文、动作在版面内形成左下聚类，版面局部压暗／柔化；HUD 位于叠层外继续可见。难度使用错位的大号英文等级文字与可读中文说明；自机页突出当前 `.selected` 立绘，另一项保留为无框文字选择；当前项最突出。未知的美术、字体与时序仍待用户确认。
- 移动端保持现有版面宽度与固定 `450×600` 逻辑尺寸，HUD 与操作按钮置于画布下方；不借本轮视觉精修新增机制、输入或模式。
