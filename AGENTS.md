...
- 自机选择暂用两色空白人形SVG，当前人物左侧放大、说明右侧，另一人物缩小模糊并在左缘露半身。角色切换450ms。模式↔难度↔自机内容转场共用760ms时间轴与 `cubic-bezier(.22,1,.36,1)`，前进先360ms闪三次；Esc/X直接反转当前动画对象，保留进度，按住返回不反复翻转。闪烁与移动阶段分别清理。自机退场动画作用于实际人物和可见文字，屏外人物保持不可见。三页真实黑带带有 `.selection-focus-band`；转场期间隐藏，由唯一 `.selection-transition-band` 按起止中心、宽高和角度插值，前进+360度、返回−360度再补最短目标角差，完成或反转结束后清除过渡元素并恢复真实黑带。模式上下／左右切换仍累计±180度。减少动态效果时立即切换。Nomiss位于难度之前，共用彩窗背景。