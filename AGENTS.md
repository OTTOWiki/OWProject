...
- 每次战斗 UI 改动与交付都必须服务**当前精确 worktree**，进入真实 `#screen-game`；遇到可恢复加载失败先按真实流程恢复并等待进入完成。
- 在同一轮采集 `1365x900`、`1264x625`、`390x844`、`320x568` 截图，逐张对照相关 TH14/TH18 参考特征与上一次基线。
- 对比同时核验几何不变量：三栏区域仍在、`#playfield` 逻辑/显示几何仍为 `450x600`、无横向溢出、HUD 与触控入口可到达且不被遮挡。
- 若视觉对比仍偏弱，自动回到 CSS 方案调整并重新执行整轮截图；测试通过只算技术证据，不等于最终视觉批准。

  - 对照特征清单：高对比粗描边标题/白字、彩色资源符号、右侧垂直仪表秩序、大面积高饱和织物/植物/裂纹纹样、强焦点与中央弹幕可读性。
  - 若上述特征弱于参考或外围舞台仍像普通网页面板，优先调整外围 CSS 并重新跑完四视口截图循环；不通过改变画布、弹幕或真实数据来补像。