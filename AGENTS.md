...
- 难度选择采用斜向序列，`_highlightDiff` 更新各项相对焦点偏移，移动、渐隐及黑带旋转统一使用300ms `cubic-bezier(0.22, 1, 0.36, 1)`；选中项居中，减少动态效果时立即定位。独立底材为 `assets/bg/difficulty-cathedral.avif`。自机页复用PR38的单一当前立绘布局与原角色素材，主菜单未整合。各屏导航快捷键常驻提示已移除，Manual/README操作说明与录像保存状态反馈保留。
