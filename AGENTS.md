...
- 自机选择暂用 `assets/ui/player-placeholder.svg`，饮泉淡青、沙玛淡紫；旧立绘保留但不用于此页。当前人物左侧放大、说明右侧，另一人物缩小模糊并在左缘露半身。角色切换450ms。模式 → 难度 → 自机均使用统一 `cubic-bezier(.22,1,.36,1)` 转场；确认先360ms内闪三次，再进入下一页或开局，转场期间锁定重复输入；Esc/X 可取消并回到来源页且保留已选状态。减少动态效果时立即切换。仅两名角色参与导航，姓名颜色稳定；点击预览先切换，确认当前角色开局。Nomiss 位于难度之前，普通模式与 Nomiss 共用难度和自机页背景。