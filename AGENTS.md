...
- 自机选择暂用 `assets/ui/player-placeholder.svg`，饮泉淡青、沙玛淡紫；旧立绘保留但不用于此页。当前人物左侧放大、说明右侧，另一人物缩小模糊并在左缘露半身。角色切换450ms。模式 → 难度 → 自机使用 `cubic-bezier(.22,1,.36,1)` 转场；确认先360ms内闪三次。转场锁定重复确认，Esc/X记录一次返回请求，当前转场完成后完整播放反向转场并保留选择。屏外选项退场时保持不可见，每项按实际边界移出，避免Lunatic返回时Easy穿过视区。模式支持上下／左右切换，黑带按方向累计±180度。减少动态效果时立即切换。仅两名角色参与自机导航；点击预览先切换，确认当前角色开局。Nomiss位于难度之前，共用彩窗背景。