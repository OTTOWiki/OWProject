...
export function mid_crossLanes(g) {
  // 纯辅压十字落；waveCount 仅相位
  installMidWave(g, {
    continuous: (game, dt) => {
      game.waveTimer = (game.waveTimer || 0) + dt;
      if (game.waveTimer < exFire(0.7)) return;
      game.waveTimer = 0;
...
export function mid_hLaserRain(g) {
  ...
  installMidWave(g, {
    continuous: (game, dt) => {
      ...
      if (game.laserT > exFire(0.85)) {
        game.laserT = 0;
        game.waveCount = (game.waveCount || 0) + 1;
        const y = 90 + (game.waveCount * 48) % 320;
        spawnHLaser(game, y, game.waveCount % 2 === 0 ? 1 : -1, C.red);
      }
...
export function mid_centerSides(g) {
  ...
  installMidWave(g, {
    ...
    maxWaves: 5,
    onWave: (game, wave) => {
      const side = wave % 2 ? 55 : LOGICAL_W - 55;
...
export function mid_gapWall(g) {
  // 无限缝墙波（章时约 12s）；maxWaves 保底高于章内可达次数
  installMidWave(g, {
...
export function mid_randomGap(g) {
  ...
  installMidWave(g, {
    ...
    maxWaves: 5,
    continuous: (game, dt) => {
      game.gapTimer = (game.gapTimer || 0) + dt;
      if (game.gapTimer > exFire(1.5)) {
        game.gapTimer = 0;
        game.gapX = 70 + Math.random() * (LOGICAL_W - 140);
      }
    },
    onWave: (game) => {
      const gap = 70;
      const gapX = game.gapX;
      for (let x = 16; x < LOGICAL_W - 16; x += 20) {
        if (x > gapX - gap / 2 && x < gapX + gap / 2) continue;
        game.spawnBullet(acquireBullet({
          x, y: -12, vx: 0, vy: exSp(1.9),
...
export function mid_rainSniper(g) {
  installMidWave(g, {
    ...
    continuous: (game, dt) => {
      game.rainT = (game.rainT || 0) + dt;
      if (game.rainT > exFire(0.3)) {
        game.rainT = 0;
        spawnGravityRain(game, 1, 'rice', C.violet, exSp(1.7));
      }
    },
    onWave: (game) => {
      const e = elite({
...
export function mid_columnLane(g) {
  installMidWave(g, {
    ...
    maxWaves: 6,
    continuous: (game, dt) => {
      game.aimTimer = (game.aimTimer || 0) + dt;
      if (game.aimTimer > exFire(0.8)) {
        game.aimTimer = 0;
...
export function mid_laserDot(g) {
  installMidWave(g, {
    ...
    onWave: (game, wave) => {
      const e = mob(50 + (wave * 80) % (LOGICAL_W - 100), 50, exHp(38), C.cyan);
      e.vy = 0.2;
      e.script = (en, d, gm) => {
        timer(en, 'L', exFire(1.2), d, () => spawnAimedLaser(gm, en, gm.player, C.cyan));
      };
      game.spawnEnemy(e);
    },
  });
  ...
  installMidWave(g, {
    ...
    continuous: (game, dt) => {
      ...
      if (game.rainT > exFire(0.3)) {
        game.rainT = 0;
        for (let i = 0; i < 3; i++) {
          spawnGravityRain(game, 1, 'rice', C.violet, exSp(1.8));
        }
      }
    },
    onWave: (game) => {
