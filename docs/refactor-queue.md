...
| 9 | **E04** | ~~完成~~（已 push） | 与关卡无关，放后；累了可穿插 |
...
| E06c | `playfieldBg` 调色板数据外置 | `MODE_THEME` → `playfieldBgThemes.js`；绘制逻辑仍在 `playfieldBg.js` | **完成**（已 push） |

Phase E 含可选 backlog 已全部收尾。
...
| 2026-07-20 | E06c | 全绿 | 完成 | `9b74309` 已 push；Phase E 全收尾 |
...
**Playwright 用例覆盖**：boot 无错误 / EX 冒烟（敌弹/分数/canvas）/ 暂停继续 / 录像保存-列表-播放-删除 / 设置持久化 / 排行榜 A线+EX续标签 / 自机选择两名角色与已移除入口检查。Nomiss 重开、进度、结算与资源回滚由 `test/cases-nomiss.js` 直接覆盖。
...
### H02 Nomiss 入口迁移到难度选择之前
| | |
|--|--|
| **状态** | 待做 |
| **目标** | 将 Nomiss 入口从当前自机选择流程迁移到难度选择之前，并恢复真实可达的 Nomiss 开局路径。 |
| **范围** | 入口与难度导航、进度续接、玩家可见说明及对应 E2E；沿用现有 Nomiss 核心机制与 `test/cases-nomiss.js` 覆盖。 |
| **当前状态** | 本轮暂时移除入口，不新增替代入口；两名自机选择、难度页返回与 Esc/X 导航保持当前契约。 |
| **验收（自动）** | 新入口能启动真实 `mode: 'nomiss'` 对局；现有 Nomiss 机制测试与相关 E2E 全绿。 |
| **验收（手测）** | 主菜单 → 难度前 Nomiss 入口 → 难度 → 两名自机选择 → 被弹重开、暂停结算、进度续接；确认背景与普通难度/自机页一致。 |
