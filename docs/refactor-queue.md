...
**Playwright 用例覆盖**：boot 无错误 / Story 模式 → 难度 → 自机真实开局 / 模式与难度返回保留选择 / 转场确认取消不延迟开局 / EX 冒烟（敌弹/分数/canvas）/ 暂停继续 / 录像保存-列表-播放-删除 / 设置持久化 / 排行榜 A线+EX续标签 / Nomiss 难度前入口、已保存进度续接、暂停无录像与结算不进榜。
...
|--|--|
| **状态** | 完成 |
| **目标** | 将 Nomiss 入口从自机选择流程迁移到难度选择之前，并恢复真实可达的 Nomiss 开局路径。 |
| **范围** | 入口与难度导航、进度续接、玩家可见说明及对应 E2E；沿用现有 Nomiss 核心机制与 `test/cases-nomiss.js` 覆盖。 |
| **验收（自动）** | Playwright 覆盖普通模式与 Nomiss 的真实启动链路、返回保留状态、确认取消；Nomiss 已保存章节续接，暂停无录像入口、结算不进排行榜。 |
| **验收（手测）** | 主菜单 → 模式 → 难度 → 自机；Nomiss 有进度时从保存章节续接；Esc/X 返回保留选择；减少动态效果即时切换。 |
