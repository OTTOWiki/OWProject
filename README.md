...
模式选择位于 Start Game 之后、难度之前：普通模式与 Nomiss 各显示名称、等级和说明；选中项居中，其他项上下错落渐隐，黑带按方向累计旋转。模式 → 难度 → 自机的确认转场可用 Esc／X 取消并回到来源页且保留选择；减少动态效果时立即切换。
...
暂停菜单位于画布左下。返回主菜单、重试、结算或续关时需二次确认，默认选“否”；Esc 返回原菜单，方向键选择、Z/Enter 确认，亦可点击或触屏选择。模式 → 难度 → 自机的选择转场可用 Esc/X 取消并回到来源页，保留已选状态；确认闪烁或转场期间重复输入会被锁定。开启减少动态效果时关闭菜单抖动、藤蔓摆动与转场动画。
