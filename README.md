...
本地开发范围：当前产品分支基于 `f723d09` 继续实现已知 UI 目标，独立于开放的 #34 与另行记录的 #35；这只是本地进行中工作，不是版本发布说明。暂停／结果在战斗版面内保留标题、正文与动作的左下聚类，并局部压暗／柔化版面，HUD 留在叠层外；当前选择、当前立绘与文字保持无框且清晰。移动端维持现有版面宽度，HUD 位于画布下方，逻辑尺寸仍为 450×600。最终美术、字体与时序等待用户确认。
