...
本地开发范围：当前产品 PR 分支干净地直接基于 `origin/main`，`f723d09` 仅作为推导已知 UI 目标内容的旧首轮 UI／内容历史来源，不是当前基线；独立于开放的 #34 与另行记录的 #35；这只是本地进行中工作，不是版本发布说明。暂停／结果在战斗版面内保留标题、正文与动作的左下聚类，并局部压暗／柔化版面，HUD 留在叠层外；难度使用错位的大号英文等级文字与可读中文说明，自机页突出当前立绘并保留另一项无框文字选择。移动端维持现有版面宽度，HUD 位于画布下方，逻辑尺寸仍为 450×600。最终美术、字体与时序等待用户确认。
