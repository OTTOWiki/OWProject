...
模式选择位于 Start Game 之后、难度之前：普通模式与 Nomiss 各显示名称、等级和说明；上下或左右键均可切换。进入下一页途中按 Esc／X 返回；返回途中按 Z／Enter 确认可重新前进。改变方向时从当前画面位置重新以先快后慢的速度播放，保留选择。黑带在内容下层连续变形，自机页按当前说明文字实际换行高度适配；减少动态效果时立即切换。
...
暂停菜单位于画布左下。返回主菜单、重试、结算或续关时需二次确认，默认选“否”，Esc 返回原菜单，方向键选择、Z/Enter 确认，亦可点击或触屏选择。选择页转场锁定重复输入，返回途中使用确认键重新前进。开启减少动态效果时关闭菜单抖动、藤蔓摆动与转场动画。
